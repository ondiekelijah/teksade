...
export default function CommmunityCard(community: CommmunityCardProps) {
  ...
  const [logoImage, loading] = useDownloadURL(
    ref(storageBucket, `logos/${community.logoUrl}`),
  );
  return (
    <Link
      href={{
    ...
    >
      <Paper
        key={community.id}
        className="group relative mb-8 rounded-lg shadow-lg"
      >
        <div className="relative ">
          <LoadingOverlay visible={loading} />
          <Text
            size="sm"
            className="absolute top-5 hidden w-full px-2 text-center group-hover:line-clamp-[8]"
          >
          ...
          <img
            ...
            alt="Cover Photo"
            className="h-56 w-full rounded-t-lg object-cover group-hover:opacity-20"
          />
        </div>
        ...
        <div className="p-4">
          <Text
            color="dimmed"
            className="-ml-0.5 flex items-center text-xs font-bold"
          >
            <LocationIcon />
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="my-4 flex items-center text-lg">
              <BsPerson className="" />{" "}
              <span className="text-xs ">{community.members}</span>
            </span>
          </div>
