...
export default function Header() {
  ...
  const RenderButton: React.FC<RenderButtonProps & { className?: string }> = ({
    href,
    children,
    className,
  }) => (
    <Link
      href={href}
      className={`mr-3 w-full whitespace-nowrap py-2 text-base sm:px-0 ${
        dark ? "hover:text-[#00afef]" : "hover:text-[#1A56DB]"
      } ${className ?? className}`}
      onClick={close}
    >
      {children}
  ...
  return (
    <Paper
      className={`sticky-header container relative mx-auto flex max-w-screen-xl flex-wrap items-center justify-between bg-transparent px-4 py-3 sm:px-8 lg:justify-between lg:px-12`}
    >
        ...
        <section className="mr-4 hidden items-center space-x-10 sm:flex">
          ...
          {userStatus.user ? (
            <>
              ...
              >
                <Menu.Target>
                  <button className="cursor-pointer border-none bg-transparent outline-none hover:bg-transparent focus:outline-none active:outline-none">
                    Account
                  </button>
                </Menu.Target>
                <Menu.Dropdown>
                  ...
                  <Menu.Item>
                    <RenderButton href="/communities/created">
                      Created Communities
                    </RenderButton>
                  </Menu.Item>
                  <Menu.Item>
                    <RenderButton href="/communities/joined">
                      Joined Communities
                    </RenderButton>
                  </Menu.Item>
                  {userIsAdmin && (
      ...
      >
        <section className="flex flex-col space-y-6 pl-0">
          <RenderButton href="/communities">Communities</RenderButton>
          <RenderButton href="/about">About Us</RenderButton>
          {userStatus.user && (
            <div className="py-2">
