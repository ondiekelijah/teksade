...
export default function CommunityUpdateModal({
  ...
  function changeProfileImage() {
    if (newProfileImage) {
      const oldProfileRef = ref(
        storageBucket,
        `logos/${communityInfo.data?.logo_link}`,
      );
    ...
    <div className="rounded-lg p-6 shadow-md">
      <div className="mb-8 flex flex-col items-center gap-4">
        <div className="mb-4 flex h-56 w-full justify-center">
          <img
            src={
              newProfileImage && isValidImageType
                ? URL.createObjectURL(newProfileImage)
                : logoImage
            }
            alt="cover-photo"
            className="h-full rounded-md object-cover shadow-sm"
          />
        </div>
        <FileInput
          ...
          error={
            hasInteracted && !isValidImageType
      ...
      <form
        onSubmit={updateForm.onSubmit((values) => handleUpdate(values))}
      ...
      >
        ...
        />
        <Select
          {...updateForm.getInputProps("focusArea")}
          size="md"
        ...
        <div className="mb-4 grid grid-cols-1 gap-2 sm:grid-cols-2">
          ...
          />
          <TextInput
            {...updateForm.getInputProps("twitter")}
            size="md"
            label="Twitter Profile URL"
          />
          <TextInput
            {...updateForm.getInputProps("linkedin")}
            size="md"
            label="LinkedIn Profile URL"
          />
          <TextInput
            {...updateForm.getInputProps("website")}
            size="md"
            label="Website URL"
          />
          <TextInput
            {...updateForm.getInputProps("whatsapp")}
            size="md"
            label="WhatsApp Group Link"
          />
          <TextInput
            {...updateForm.getInputProps("phone")}
            size="md"
            label="Contact Number"
          />
          <TextInput
            {...updateForm.getInputProps("youtube")}
            size="md"
            label="YouTube Channel Link"
          />
          <TextInput
            {...updateForm.getInputProps("slack")}
            size="md"
            label="Slack Group Link"
          />
          <TextInput
            {...updateForm.getInputProps("discord")}
            size="md"
            label="Discord Group Link"
          />
          <TextInput
            {...updateForm.getInputProps("telegram")}
            size="md"
            label="Telegram Group Link"
          />
          <TextInput
            {...updateForm.getInputProps("meetup")}
            size="md"
            label="Meetup Group Link"
          />
        </div>
        ...
        <div className="my-6 flex justify-center">
          <CustomButton
            ...
            type="submit"
            title={
              updateCommunity.isLoading || communityInfo.isLoading
                ? "Updating..."
                : "Update"
            }
            disabled={
              !updateForm.isValid() ||
              updateCommunity.isLoading ||
