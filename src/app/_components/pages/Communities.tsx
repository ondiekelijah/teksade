...
export default function CommunitiesPage() {
  ...
  const [selectedTechnologies, setSelectedTechnologies] = useState<
    string[] | undefined
  >(undefined);
  const [selectedFocusAreas, setSelectedFocusares] = useState<
    string[] | undefined
  >(undefined);
          ...
          <Collapse in={filtersOpen}>
            <div className="my-5 flex w-full flex-col  gap-2 sm:flex sm:flex-row sm:items-center sm:justify-between">
              ...
              <Select
                ...
                onChange={(val) => setSelectedCountry(val ?? undefined)}
                className=" flex-1"
              />
            </div>
          </Collapse>
          ...
          {communitiesList.data?.length
            ...
            : !communitiesList.isLoading && (
                <div className="my-20 text-center sm:col-span-3 md:col-span-4">
                  It looks like there aren&apos;t any communities that fit those
                  specifics. Don&apos;t worry—adjusting your filters might help!
                </div>
              )}
        </section>
        {mightHaveMore && (
