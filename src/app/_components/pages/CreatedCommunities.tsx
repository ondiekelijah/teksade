...
export default function CreatedCommunitiesPage() {
  ...
  const communitiesCreatedByMember =
    api.members.getCommunitiesCreatedByMember.useQuery({
      memberId: user?.id ?? "",
  ...
  const deleteCreatedCommunity = api.communities.deleteCommunity.useMutation({
    onSuccess: () => {
      void queryClient.members.getCommunitiesCreatedByMember.refetch({
        memberId: user?.id ?? "",
      });
    },
  });
  ...
  function handleDelete(communityID: string) {
    openConfirmModal({
      ...
      children:
        "Deleting this community means all related data will be lost forever.",
  ...
  function handleUpdate(communityID: string) {
    openModal({
      ...
      size: "100%",
      overlayProps: {
        color: true ? theme.colors.dark[9] : theme.colors.gray[2],
        opacity: 0.55,
        blur: 3,
      },
    });
            ...
            {communitiesCreatedByMember.data?.map((community) => (
              <div key={community.id}>
                <CommmunityCard
                  ...
                  name={community.name}
                  members={
                    community._count.members ? community._count.members : 0
                  }
                  ...
                  actionButtons={
                    <div className="flex w-full items-center justify-between">
                      <CustomButton
                        ...
                        variant="filled"
                        title="Delete"
                        onClickHandler={() => handleDelete(community.id)}
                        color="red"
                      />
                      <CustomButton
                        size="sm"
                        variant="filled"
                        title="Update"
                        onClickHandler={() => handleUpdate(community.id)}
                      />
                    </div>
                  }
                />
              ...
              <div className="my-20 text-center sm:col-span-3 md:col-span-4">
                ...
                <div className="my-6 flex justify-center">
                  <CustomButton
                    ...
                    type="submit"
                    title="Add Community"
                    onClickHandler={() => void router.push("/communities/new")}
                  />
                </div>
              </div>
            )}
