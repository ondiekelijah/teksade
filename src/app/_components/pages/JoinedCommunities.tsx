...
export default function JoinedCommunities() {
  ...
  const communitiesJoinedByMember =
    api.members.getCommunitiesJoinedByMember.useQuery({
      memberId: user?.id ?? "",
  ...
  return (
    <Container>
      <SectionTitle
        heading="My Tribes"
        description="The pulse of your digital life – all your communities at your fingertips"
      />
      ...
      {communitiesJoinedByMember.isLoading ? (
        ...
        <div className=" grid grid-cols-1 gap-2 p-2 sm:grid-cols-3">
          {communitiesJoinedByMember.data?.communities_joined.map(
            (community) => (
        ...
        <div className="my-20 text-center sm:col-span-3 md:col-span-4">
          ...
          <div className="my-6 flex justify-center">
            <CustomButton
              ...
              onClickHandler={() => void router.push("/communities")}
            />
          </div>
        </div>
      )}
        ...
        <div className="my-20 text-center sm:col-span-3 md:col-span-4">
          {searchTerm ? (
            <>
              ...
              <span className={`${dark ? "text-[#00afef]" : "text-[#1A56DB]"}`}>
                {searchTerm}
              </span>
