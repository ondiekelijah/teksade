...
export default function NewCommunityPage() {
  ...
  const { user } = useUser();
  const getMemberInfo = api.members.getMemberInfo.useQuery({
    memberId: user?.id ?? "",
  });
  ...
  async function handleLogoUpload() {
    if (profileImage) {
      await uploadFile(
        ref(
          storageBucket,
          `logos/${form.values.communityName.split(" ").join("")}`,
        ),
        profileImage,
      );
    }
        ...
        await createNewCommunity
          ...
          .then((onfulfilledValue) => {
            if (onfulfilledValue?.country) {
              ...
              notifyError({
                message:
                  "Hang tight! We faced a glitch while creating your community.",
              });
            }
          });
      } catch (error) {
  ...
  if (
    !getMemberInfo.isLoading &&
    !(getMemberInfo.data?.name ?? getMemberInfo.data?.email)
  ) {
    return (
      <div className="flex w-full flex-col items-center justify-center space-y-4 py-10 text-center">
        <div
          className={`mb-4 flex items-center rounded-lg p-4 text-sm ${
            dark
              ? "border-blue-800 bg-gray-800 text-blue-400"
              : "border-blue-300 bg-blue-50 text-blue-800"
          }`}
          role="alert"
        >
          <svg
            className="mr-3 inline h-10 w-10 flex-shrink-0"
            aria-hidden="true"
          ...
          <div>
            <span className="text-base">
              Let&apos;s get to know you better! Complete your basic profile
              before adding a community.
            </span>
          </div>
        </div>
        <Link href="/profile">
  ...
  return (
    <Container>
      ...
      />
      <form
        onSubmit={form.onSubmit((values) => void handleNewCommunity(values))}
        className="flex animate-slideInDown flex-col gap-2"
      >
        <Stepper
          active={active}
          onStepClick={setActive}
          allowNextStepsSelect={false}
          className=" mx-auto my-auto mt-10 w-full p-4 sm:w-[60vw]"
          id="stepper"
        >
          <Stepper.Step
            label="Step 1"
          ...
          <Stepper.Step label="Step 3" description="Image Upload">
            <div className="flex flex-col gap-4 pt-4">
              ...
              <CustomButton
                ...
                type="submit"
                title={
                  createNewCommunity.isLoading || uploading
                    ? "Creating ..."
                    : "Add Community"
                }
          ...
          <Stepper.Completed>
            <div className="flex flex-col items-center justify-center space-y-4 py-10 text-center">
              Great job! Your community has been created. We&apos;ll publish it
              once it&apos;s approved.
            </div>
          </Stepper.Completed>
        </Stepper>
