...
const StickyBanner = () => {
  ...
  const expireDate = new Date(
    createdDate.setDate(createdDate.getDate() + duration),
  );
  ...
  if (!published || isExpired || targetPage !== "all") return null;
  ...
  return (
    <Box
      className={`text-muted-foreground flex items-center justify-center p-1 text-center ${bgColor} w-full`}
    >
      <div className="mx-auto flex items-center">
        <p className={`flex items-center text-sm font-normal ${textColor}`}>
          <span
            className={`mr-3 hidden h-6 w-6 items-center justify-center rounded-full p-1 lg:block ${bulbBgColor}`}
          >
            <LightBulbIcon />
          ...
          <span>
            ...
            <span
              ...
              className={`text-sm font-normal ${textColor}`}
            ></span>
          </span>
        </p>
      </div>
      <div className="ml-4 flex items-center">
        <span
          className="inline-flex h-7 w-7 flex-shrink-0 cursor-pointer items-center justify-center"
